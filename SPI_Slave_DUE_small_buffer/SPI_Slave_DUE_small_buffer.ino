...
static int aryInBuffer[8] = {0, 0, 0, 0, 0, 0, 0, 0,};
static int aryOutBuffer[8] = {1, 2, 3, 4, 5, 6, 7, 9,};
...
//    Serial.print(iInBufferIdx);
    Serial.print(" Buffer: ");
    Serial.println(aryInBuffer[iInBufferIdx], DEC);
...
void loop() {
  ...
  if (iOldInBufferIdx != iInBufferIdx) {
    ...
    Serial.println(iInBufferIdx, DEC);
  ...
  if (haveData) {
    ...
    for (int iIdx = 0; iIdx < 10; iIdx++) {
      Serial.print((byte)aryInBuffer[iIdx], DEC);
      Serial.print(", ");
    }
