...
void T3SPI::enablePins(uint8_t sck, uint8_t mosi, uint8_t miso, uint8_t cs, bool activeState) {
  ...
  }  //Set Pin13 Output & SCK
  if (sck == 0x0E) {
    CORE_PIN13_CONFIG = PORT_PCR_MUX(1);
    CORE_PIN14_CONFIG = PORT_PCR_DSE | PORT_PCR_MUX(2);
  }  //Set Pin14 Output & SCK
  if (mosi == 0x0B) {
    CORE_PIN11_CONFIG = PORT_PCR_DSE | PORT_PCR_MUX(2);
  }  //Set Pin11 Output & MOSI
  if (mosi ==  0x07) {
    CORE_PIN7_CONFIG  = PORT_PCR_DSE | PORT_PCR_MUX(2);
  }  //Set Pin7  Output & MOSI
  if (miso == 0x0C) {
    CORE_PIN12_CONFIG = PORT_PCR_MUX(2);
  }         //Set Pin12 Input & MISO
  if (miso ==  0x08) {
...
void T3SPI::rx8(volatile uint8_t *dataIN, int length) {
  dataIN[dataPointer] = (byte)SPI0_POPR;
  Serial.print("["); Serial.print(dataPointer); Serial.print("] = "); Serial.println(dataIN[dataPointer]);
  Serial.flush();
  dataPointer++;
